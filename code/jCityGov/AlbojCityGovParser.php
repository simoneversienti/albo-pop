...
class AlbojCityGovParser implements Iterator{
	...
	private static function getTableElement2($page){
		$table=AlbojCityGovParser::getElementByTagName($page->documentElement, 'table');
	...
	private static function getElementByTagName($root, $tagname){
		...
		for($i=0; $i<$root->childNodes->length; $i++){
			...
			$r=AlbojCityGovParser::getElementByTagName($child, $tagname);
			if (isset($r))
				return $r;
		}
		return null;
