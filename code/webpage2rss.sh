...
curl -s -b cookies.txt  -L 'http://pubblicazioni.saga.it/publishing/AP/index.do?dispatch=search&d-5113963-p=2&org=pedara' -s | /usr/local/bin/nadir/scrape -be '#documentList > tbody > tr:not(:first-child)' | tr '\n' ' ' | xml2json | jq '[.html.body.tr[] | {pubDate:.td[1]."#text",title:.td[4]."#text",id:.td[7].a."@href"}]' | in2csv -f json  > $cartella/02.csv
curl -s -b cookies.txt  -L 'http://pubblicazioni.saga.it/publishing/AP/index.do?dispatch=search&d-5113963-p=3&org=pedara' -s | /usr/local/bin/nadir/scrape -be '#documentList > tbody > tr:not(:first-child)' | tr '\n' ' ' | xml2json | jq '[.html.body.tr[] | {pubDate:.td[1]."#text",title:.td[4]."#text",id:.td[7].a."@href"}]' | in2csv -f json  > $cartella/03.csv
